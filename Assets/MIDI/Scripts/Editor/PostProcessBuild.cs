...
namespace jp.kshoji.unity.midi.Editor
{
...
#else
            androidManifest.RemoveFeature("android.software.companion_device_setup");
#endif
    ...
    internal class AndroidManifest : XmlDocument
    {
        ...
        internal void RemoveFeature(string feature)
        {
            if (!HasFeature(feature))
            {
                return;
            ...
            {
                if (((XmlNode)childNode).Name != "uses-feature")
                {
                    continue;
                }

                var attributes = ((XmlNode)childNode).Attributes;
                if (attributes?[nameAttribute.Name] != null && attributes[nameAttribute.Name].Value == feature)
                {
                    // remove this element
                    manifest.RemoveChild((XmlNode)childNode);
                    UnityEngine.Debug.LogWarning($"The feature `android.software.companion_device_setup` has been removed from AndroidManifest.xml.");
                    return;
                }
            }
        }
