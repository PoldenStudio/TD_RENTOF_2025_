...
namespace jp.kshoji.midisystem
{
    ...
    public class StandardMidiFileReader
    {
        ...
        public Sequence GetSequence(Stream inputStream, bool ignoreErrors = false)
        {
                        ...
                        try
                        {
                            ...
                            if (data < 0x80)
                            {
                                ...
                                else
                                {
                                    ...
                                    throw new InvalidMidiDataException($"Invalid data. runningStatus: {runningStatus:X2}, data: {data:X2}, position: {{midiDataInputStream.Position:X2}}");
                                }
                            }
                            else if (data < 0xf0)
                            {
                                // Control messages
                                message = ProcessRunningMessage(data, midiDataInputStream.ReadByte(),
                                    midiDataInputStream);
