...
namespace jp.kshoji.midisystem
{
    ...
    public static class MidiSystem
    {
        ...
        /// <param name="ignoreErrors">ignore errors on reading SMF</param>
        /// <returns><see cref="Sequence"/></returns>
        public static Sequence ReadSequence(Stream stream, bool ignoreErrors = false)
        {
            using (stream)
            {
                return midiFileReader.GetSequence(stream, ignoreErrors);
            }
        }
