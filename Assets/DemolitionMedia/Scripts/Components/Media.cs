...
namespace DemolitionStudios.DemolitionMedia
{
    ...
    public partial class Media : IDisposable
    {
        ...
        /// Extract compressed Snappy/GDeflate frames and do compute shader decompression
        /// Only works with Pro Sync asset version
        public bool GPUComputeBasedDecompression = false;
#endif
        ...
        {
            get { return NativeDll.CanPlay(_mediaId) != 0
                         && !IsOpening
                         && !IsClosing; }
        }
        ...
        {
            get { return NativeDll.IsPlaying(_mediaId) != 0; }
        }

        /// Whether the media is currently has non-zero number of loops remaining
        public bool IsLooping
        {
            get { return NativeDll.IsLooping(_mediaId) != 0; }
        }

        /// Whether the media is finished playing
        public bool IsFinished
        {
            get { return NativeDll.IsFinished(_mediaId) != 0; }
        }
        ...
        {
            get { return NativeDll.GetFramedropEnabled(_mediaId) != 0; }
            set { byte v = Utilities.toByte(value); NativeDll.SetFramedropEnabled(_mediaId, v); }
        }
        ...
        /// Sync mode
        public FramePresentMode VideoFramePresentMode
        {
            get { return NativeDll.GetVideoFramePresentMode(_mediaId); }
        }
        ...
        {
            get { return TexturesCreated() && NativeDll.NativeTexturesFirstFrameUploaded(_mediaId) != 0; }
        }
                         ...
                         GraphicsInterfaceDeviceType deviceType = GraphicsInterfaceDeviceType.Unknown, IntPtr devicePtr = default)
        {
            ...
            generalParams.enableHeaderMagicProtection = Utilities.toByte(enableHeaderMagicProtection);
            generalParams.preloadToMemory = Utilities.toByte(preloadToMemory);
            ...
            audioParams.enableAudio = Utilities.toByte(openWithAudio);
            audioParams.useNativeAudioPlugin = Utilities.toByte(useNativeAudioPlugin);

            bool useSrgbCompressedTextureFormats = false;
            FramePresentMode framePresentMode;
            GetGraphicsParams(out framePresentMode, out useSrgbCompressedTextureFormats);
#if UNITY_5_3_OR_NEWER
            var gfxDevice = SystemInfo.graphicsDeviceType;
#if UNITY_2022_1_OR_NEWER
            if (framePresentMode == FramePresentMode.ImmutableTextures)
            {
                Utilities.LogError("Unity 2022 or newer detected, but passRawTextureDataOutside=false");
                return false;
            }
            if (GPUComputeBasedDecompression)
            {
#if !(UNITY_STANDALONE_WIN || UNITY_EDITOR_WIN)
                Debug.LogError("For GPU decoding only Windows supported at the moment");
                InvokeEvent(MediaEvent.Type.GPUComputeBasedDecompressionNotAvailable);
                return false;
#else
                if (NativeDll.IsProVersion() == 0) 
                {
                    Debug.LogError("For GPU decoding a Pro Sync asset version is needed");
                    InvokeEvent(MediaEvent.Type.GPUComputeBasedDecompressionNotAvailable);
                    return false;
                }
                else if (SystemInfo.graphicsDeviceType != GraphicsDeviceType.Direct3D12)
                {
                    Debug.LogError("For GPU decoding Direct3D12 is required. Enable it in the Project Settings -> Player -> Graphics API for Windows, or add -force-d3d12 to Unity Hub Editor command line arguments");
                    InvokeEvent(MediaEvent.Type.GPUComputeBasedDecompressionNotAvailable);
                    return false;
                }
#endif
...
#else
            if (framePresentMode == FramePresentMode.DynamicTextureFallback)
            {
                Utilities.LogError("fallbackToDynamicTexture=true which may lead to memory leaks on open videos or even crashes");
                return false;
            }
            if (gfxDevice != GraphicsDeviceType.Direct3D11 && framePresentMode != FramePresentMode.PassRawTextureDataOutside)
            {
            ...
            giParams.useSrgbCompressedTextureFormats = Utilities.toByte(useSrgbCompressedTextureFormats);
            giParams.framePresentMode = framePresentMode;

            var openingStarted = NativeDll.Open(_mediaId, generalParams, audioParams, giParams) != 0;
            if (!openingStarted)
            {
                IsOpening = false;
        ...
        public bool Muted
        {
            get { return NativeDll.GetAudioMuted(_mediaId) != 0; }
        }

        public void SetMuted(bool muted)
        {
            NativeDll.SetAudioMuted(_mediaId, Utilities.toByte(muted));
        }

        public bool AudioEnabled
        {
            get { return NativeDll.GetAudioEnabled(_mediaId) != 0; }
        }
        ...
        private void OnOpened()
        {
...
#if UNITY_2022_1_OR_NEWER
            if (VideoPixelFormat == PixelFormat.YCoCgAndAlphaP)
                Utilities.LogError("Unity 2022+ detected, and Hap Q Alpha video format is used. The result is likely to be invalid. Please use Hap Alpha or Hap R with alpha formats instead");
#endif

            HasAudio = NativeDll.HasAudioStream(_mediaId) != 0;
            ...
            bool hasHardwareAcceleration = NativeDll.IsDecodingHardwareAccelerated(_mediaId) != 0;
        ...
        private void PopulateErrors()
        {
            ...
            if (error != MediaError.NoError)
            {
                if (IsOpening)
                {
                    ...
                    Utilities.LogError("[DemolitionMedia] Path: " + mediaUrl + " | OpenFailed: " + error.ToString());
#if UNITY_5_3_OR_NEWER
